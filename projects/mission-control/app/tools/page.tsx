...
export default function ToolsPage() {
  return (
    <div>
      ...
      <p className="text-slate-500 text-sm mb-8">Launch linked tools and dashboards.</p>

      <div className="grid grid-cols-2 gap-4">
        {tools.map(({ title, desc, href, icon: Icon, color, bg }) => (
          <div
            key={href}
            className="bg-slate-50 border border-slate-200 rounded-xl p-6 flex flex-col gap-4 hover:border-blue-500/50 hover:shadow-md transition-all"
          >
