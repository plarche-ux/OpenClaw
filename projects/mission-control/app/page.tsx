    ...
    <div>
      ...
      <div className="grid grid-cols-3 gap-4 mb-10">
        <div className="bg-slate-50 rounded-xl border border-slate-200 p-5 hover:border-blue-400 transition-all">
          ...
          <div className="text-3xl font-bold text-slate-900 font-mono">{info?.mdCount ?? '—'}</div>
          <div className="text-xs text-slate-500 mt-1">.md files indexed</div>
        </div>
        <div className="bg-slate-50 rounded-xl border border-slate-200 p-5 hover:border-blue-400 transition-all">
          <div className="flex items-center gap-2 text-slate-500 text-xs uppercase tracking-wide mb-1">
          ...
          <div className="text-xs text-slate-500 mt-1">since last restart</div>
        </div>
        <div className="bg-slate-50 rounded-xl border border-slate-200 p-5 hover:border-blue-400 transition-all">
          <div className="flex items-center gap-2 text-slate-500 text-xs uppercase tracking-wide mb-1">
      ...
      <div className="grid grid-cols-2 gap-3 lg:grid-cols-4">
        {sections.map(({ href, label, icon: Icon, desc }) => (
          <Link
            key={href}
            href={href}
            className="bg-slate-50 rounded-xl border border-slate-200 p-5 hover:border-blue-500 hover:shadow-md transition-all group"
          >
            <Icon size={20} className="text-blue-600 mb-3" />
            <div className="font-semibold text-slate-900 group-hover:text-blue-600 mb-1 transition-colors">{label}</div>
