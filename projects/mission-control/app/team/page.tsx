...
const ROLE_BADGE: Record<string, { bg: string; text: string; dot: string }> = {
  'primary-agent': { bg: 'bg-blue-50',  text: 'text-blue-600',  dot: 'bg-blue-600 text-white'  },
  'book-agent':    { bg: 'bg-blue-600 text-white/10',  text: 'text-blue-600',  dot: 'bg-blue-600 text-white'  },
...
export default function TeamPage() {
  ...
  return (
    <div className="relative">
      ...
      <p className="text-slate-500 text-sm mb-8">Agents and collaborators.</p>
      ...
      <div className="grid grid-cols-2 gap-4 lg:grid-cols-3">
        {agents.map(agent => {
          ...
          return (
            <div key={agent.id} className="bg-slate-50 border border-slate-200 rounded-xl p-5 flex flex-col gap-3 hover:border-blue-400 transition-all">
              <div className="flex items-start justify-between">
                <div className="w-12 h-12 bg-white rounded-2xl flex items-center justify-center text-2xl border border-slate-200">
                  {agent.emoji}
                </div>
                {badge && (
                  <span className={`inline-flex items-center gap-1.5 ${badge.bg} ${badge.text} text-xs font-medium px-2.5 py-1 rounded-full border border-blue-500/20`}>
                ...
                {isAgent ? (
                  <div className="flex items-center gap-2">
                    ...
                    <span className={`text-xs font-semibold px-2 py-0.5 rounded-full font-mono ${
                      openTasks > 0 ? 'bg-blue-50 text-blue-600' : 'bg-slate-100 text-slate-500'
                    }`}>
                      {openTasks}
                    </span>
                  </div>
                ) : (
                  <span className="text-xs text-slate-500">{ROLE_LABEL[agent.role]}</span>
                )}
                {hasFiles && (
                  <button
                    onClick={() => openViewer(agent)}
                    className="inline-flex items-center gap-1 text-xs text-blue-600 hover:text-blue-700 font-medium hover:bg-blue-50 px-2 py-1 rounded-lg transition-colors"
                  >
                    <FileText size={13} />
                    Files
                  </button>
        ...
        <div className="border-2 border-dashed border-slate-200 rounded-xl p-5 flex flex-col items-center justify-center gap-2 text-slate-500 hover:border-blue-400 hover:text-blue-600 transition-colors cursor-default min-h-[160px]">
          <UserPlus size={24} />
          ...
          <div className="w-full max-w-2xl bg-white shadow-2xl flex flex-col h-full overflow-hidden border-l border-slate-200">
            ...
            <div className="flex items-center gap-3 px-5 py-4 border-b border-slate-200 bg-slate-50">
              ...
              <div className="flex-1">
                <div className="font-semibold text-slate-900">{viewerAgent.name}</div>
                <div className="text-xs text-slate-500">{viewerAgent.description ?? ROLE_LABEL[viewerAgent.role]}</div>
              </div>
              <button
                onClick={() => { setViewerOpen(false); setEditing(false) }}
            ...
            <div className="flex flex-1 overflow-hidden">
              {/* File list sidebar */}
              <div className="w-44 border-r border-slate-200 bg-slate-50 flex flex-col overflow-y-auto">
                <div className="px-3 py-2 text-xs font-semibold text-slate-500 uppercase tracking-wide">Workspace Files</div>
                {agentFiles.length === 0 && (
                ...
                {agentFiles.map(f => (
                  ...
                  >
                    <ChevronRight size={13} className={selectedFile === f.name ? 'text-blue-600' : 'text-slate-500'} />
                    {f.name}
                  </button>
                ))}
              </div>
                ...
                {!selectedFile ? (
                  ...
                  <>
                    ...
                    <div className="flex items-center justify-between px-4 py-2 border-b border-slate-200 bg-slate-50">
                      <span className="text-sm font-medium text-slate-900 font-mono">{selectedFile}</span>
                      <div className="flex items-center gap-2">
                        ...
                        {editing ? (
                          <>
                            ...
                            <button
                              onClick={saveFile}
                              disabled={saving}
                              className="inline-flex items-center gap-1 text-xs bg-blue-600 hover:bg-blue-700 text-white px-3 py-1.5 rounded-lg font-medium transition-colors disabled:opacity-50"
                            >
                              <Save size={12} />
                              {saving ? 'Saving…' : 'Save'}
                            </button>
                          ...
                          <button
                            ...
                            className="text-xs text-blue-600 hover:text-blue-700 hover:bg-blue-50 px-3 py-1.5 rounded-lg font-medium transition-colors"
                          >
                            Edit
                          </button>
                        )}
                    ...
                    <div className="flex-1 overflow-auto p-4">
                      {editing ? (
                        <textarea
                          className="w-full h-full min-h-[400px] font-mono text-sm text-slate-800 bg-slate-50 border border-slate-200 rounded-lg p-3 focus:outline-none focus:ring-2 focus:ring-blue-500/30 focus:border-blue-500 resize-none"
                          value={editContent}
